...
- `apps/homework-monster` — **宿題モンスター / HOMEWORK MONSTER**  宿題そのものがモンスターのごはんになる育成ゲーム。生まれたてのモンスターは口が小さく、「算数20問」のままでは食べられない。プレイヤーが宿題を1問・1行・ノートを開くだけの「ひとくち」に切り分け、実際に終わらせてから一口ずつ食べさせる。大きすぎたときは責めずに、もっと小さく切り直す。設計は [`QUALITY.md`](./apps/homework-monster/QUALITY.md)。
- `apps/404-kaiki` — **404怪異調査クラブ / 404**  中学生3人組「404部」で町を歩き、近づくだけで調査が進む探索型の都市伝説ホラー。操作は移動だけ。戦闘もHPも無く、怪異ごとに違う方法で生き延びる。CASE 01「口裂け女」＝夜の住宅街を**逃げ切る**、CASE 02「くねくね」＝夏の夕方の田園を**見ないで帰る**、CASE 03「テケテケ」＝放課後の学校と線路を**上がってやり過ごす**。設計は [`SPEC.md`](./apps/404-kaiki/SPEC.md)、各CASEの実装範囲は [`CASE01_SLICE.md`](./apps/404-kaiki/CASE01_SLICE.md)・[`CASE02_SLICE.md`](./apps/404-kaiki/CASE02_SLICE.md)・[`CASE03_SLICE.md`](./apps/404-kaiki/CASE03_SLICE.md)。
