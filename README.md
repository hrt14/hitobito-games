...
- `apps/404-kaiki` — **404怪異調査クラブ / 404**  中学生3人組「404部」で夜の町を歩き、近づくだけで調査が進む探索型の都市伝説ホラー。怪異を見つけたら戦わずに逃げる。設計は [`SPEC.md`](./apps/404-kaiki/SPEC.md)、最初に作る CASE 01 の5エリア縦切り版は [`CASE01_SLICE.md`](./apps/404-kaiki/CASE01_SLICE.md) を参照。現行の実装ファイルは旧方針（架空スマートフォンOSの謎解き型）で、探索型へ作り直す。
