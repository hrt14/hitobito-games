...
try {
  ...
  if (!revisitHint.includes('1回交渉') || !revisitHint.includes('時間がない')) fail(`Revisit did not remember session/resistance: ${revisitHint}`);

  // Failure path: keep rejecting until the explicit choice to stop for today.
  await page.getByRole('button', { name: /無理。そんな時間ない/ }).click();
  await page.getByRole('button', { name: /5分でも長い/ }).click();
  ...
  await page.screenshot({ path: path.join(artifacts, '04-failure-390.png'), fullPage: true });
  ...
  const summary = {
    ...
    tested: [
      ...
      'reload/revisit persistence of session and resistance tendency',
