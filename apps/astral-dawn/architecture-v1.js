...
function forgeCorner(){const x=3.5,z=19.5,g=new THREE.Group();g.position.set(x,y(x,z),z);g.rotation.y=-.35;core.world.add(g);const stone=mat(0x626965,.96),iron=mat(0x313b3d,.5,.65),fire=mat(0xffaa55,.3,0,{emissive:0xff5a22,emissiveIntensity:4});add(g,new THREE.BoxGeometry(1.8,1.05,1.25),stone,[0,.55,0]);add(g,new THREE.BoxGeometry(1.2,.2,.75),iron,[0,1.16,0]);const coal=add(g,new THREE.BoxGeometry(.95,.12,.55),fire,[0,1.3,0]);const light=new THREE.PointLight(0xff6b32,2.1,8,2);light.position.set(0,1.8,0);g.add(light);add(g,new THREE.BoxGeometry(.9,.22,.38),iron,[1.45,.9,.4]);add(g,new THREE.CylinderGeometry(.18,.3,.8,8),iron,[1.45,.42,.4]);g.userData={fire:coal,light};}
...
function shrineArchitecture(){const s=core.scene.userData.shrine;if(!s)return;const stone=mat(0x6d7b79,.95),edge=mat(0x879994,.8),rune=mat(0x9efff4,.25,.25,{emissive:0x43cfd1,emissiveIntensity:2.1});
  ...
  pylons.forEach(([px,pz,h],idx)=>{add(s,new THREE.BoxGeometry(.7,h,.9),stone,[px,h/2,pz],[1,1,1],[idx*.03,idx*.19,.04*(idx-1)]);add(s,new THREE.BoxGeometry(1.05,.24,1.2),edge,[px,h+.08,pz]);if(idx!==3)add(s,new THREE.OctahedronGeometry(.14,0),rune,[px,h+.5,pz]);});
  for(let i=0;i<8;i++){const a=i/8*Math.PI*2;add(s,new THREE.BoxGeometry(.045,.025,2.2),rune,[Math.sin(a)*1.8,.45,Math.cos(a)*1.8],[1,1,1],[0,a,0]);}
