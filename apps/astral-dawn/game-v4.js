...
let renderer, scene, camera, clock, player, companion, sun, hemi, keyLight, audioCtx;
...
const S = {
  ...
  enemy: null, enemyObj: null, moveT: 0, area: 'ミレア草原', bossSpawned: false, joined: false, turn: 0
};
...
function joinCompanion(){
  if(S.joined)return; S.joined=true;
  const old=npcs.find(n=>n.userData.name==='リナ'); if(old)old.visible=false;
  const g=new THREE.Group(),body=mesh(new THREE.CapsuleGeometry(.36,1.02,6,9),mat(0x6e455e)),head=mesh(new THREE.SphereGeometry(.36,14,10),mat(C.skin));
  body.position.y=1.4;head.position.y=2.35;g.add(body,head);
  const hood=mesh(new THREE.ConeGeometry(.5,.65,8),mat(0x423348));hood.position.y=2.75;g.add(hood);
  const staff=new THREE.Group(),rod=mesh(new THREE.CylinderGeometry(.045,.055,2.1,7),mat(C.wood)),gem=mesh(new THREE.OctahedronGeometry(.16,0),new THREE.MeshStandardMaterial({color:C.cyan,emissive:0x3bc6cf,emissiveIntensity:2}),false);
  rod.position.y=1;gem.position.y=2.1;staff.add(rod,gem);staff.position.set(.48,.2,0);staff.rotation.z=-.1;g.add(staff);g.userData={staff,gem,phase:0};
  companion=g;companion.position.copy(player.position).add(new THREE.Vector3(-1,0,1.5));companion.position.y=groundY(companion.position.x,companion.position.z)+.22;world.add(companion);
  toast('リナが仲間になった',2200);tone(660,.14);setTimeout(()=>tone(880,.22),130);
}
...
function start(){
  ...
  setTimeout(()=>dialogue('リナ',['……起きた？　よかった。流星が落ちてから、森の獣たちが妙に荒れてる。','東の〈薄明の祠〉なら、星の欠片の正体が分かるかもしれない。青い灯を追って。','剣は忘れないで。今日は、たぶん静かな朝じゃ終わらない。'],()=>joinCompanion()),1300);
}
...
function nearest(){ let best=null,bd=99;for(const i of interactables){if(!i.obj.visible)continue;const d=player.position.distanceTo(i.obj.position);if(d<i.r&&d<bd){best=i;bd=d}}return best; }
...
function battle(e){
  ...
  S.mode='transition'; S.can=false; S.turn=0; S.enemyObj=e; S.enemy={...e.userData}; e.userData.field=e.position.clone(); $('#letterbox').classList.add('active'); flash(.25); setTouch(false);
  setTimeout(()=>{ S.mode='battle';S.can=true;e.position.set(player.position.x+3.5,groundY(player.position.x+3.5,player.position.z-4.6)+.15,player.position.z-4.6);if(companion){companion.position.set(player.position.x-1.6,groundY(player.position.x-1.6,player.position.z-.3)+.22,player.position.z-.3);companion.rotation.y=player.rotation.y;}e.rotation.y=Math.PI;$('#enemyName').textContent=S.enemy.name;$('#battleHud').classList.remove('hidden');updateBattle();log(S.enemy.boss?'古代種が星光を喰らっている！':'魔獣がこちらを睨んでいる！');setTimeout(()=>$('#letterbox').classList.remove('active'),350); },550);
}
...
function command(c){
  ...
  S.turn++; const skill=c==='skill'; if(skill){S.mp-=8;updateUI();log('星断ち！')}else log('アレンの攻撃！');
  const from=player.position.clone(),to=S.enemyObj.position.clone().add(new THREE.Vector3(0,0,2));
  effects.push({type:'lunge',o:player,from,to,t:0,d:skill?.58:.4,skill,hit:false,cb:()=>{const crit=!skill&&Math.random()<.12;const n=Math.round(skill?rand(66,82):rand(30,40))*(crit?1.7:1);damage(Math.round(n),crit);setTimeout(()=>S.enemy.hp>0?companionAssist():win(),520)}});
}
...
function companionAssist(){
  if(!companion||S.enemy.hp<=0)return enemyTurn();
  if(S.turn%2!==0)return enemyTurn();
  S.can=false;
  const heal=S.hp<62;
  if(heal){
    log('リナの「星灯り」！',650);companion.userData.gem.material.emissiveIntensity=5;tone(760,.18);
    setTimeout(()=>{const n=30;S.hp=Math.min(S.maxHp,S.hp+n);updateUI();spawnHeal(player.position,n);companion.userData.gem.material.emissiveIntensity=2;setTimeout(enemyTurn,420)},420);
  }else{
    log('リナの「ルミナ」！',650);companion.userData.gem.material.emissiveIntensity=5;tone(940,.12);
    setTimeout(()=>{const n=Math.round(rand(16,23));S.enemy.hp=Math.max(0,S.enemy.hp-n);spawnDamage(S.enemyObj.position,n);camera.userData.shake=.1;updateBattle();companion.userData.gem.material.emissiveIntensity=2;if(S.enemy.hp<=0)setTimeout(win,400);else setTimeout(enemyTurn,420)},420);
  }
}

function spawnHeal(pos,n){
  const d=document.createElement('div');d.textContent=`+${n}`;Object.assign(d.style,{position:'fixed',zIndex:50,color:'#aef5d1',font:'800 24px serif',textShadow:'0 3px 10px #000',pointerEvents:'none',transition:'transform .7s,opacity .7s'});document.body.appendChild(d);
  const p=pos.clone().add(new THREE.Vector3(0,2,0)).project(camera);d.style.left=`${(p.x*.5+.5)*innerWidth}px`;d.style.top=`${(-p.y*.5+.5)*innerHeight}px`;requestAnimationFrame(()=>{d.style.transform='translateY(-48px) scale(1.2)';d.style.opacity=0});setTimeout(()=>d.remove(),750);
}
...
function endBattle(boss){
  ...
  setTimeout(()=>{ $('#battleHud').classList.add('hidden');$('#letterbox').classList.remove('active');S.mode='field';S.can=true;S.enemy=S.enemyObj=null;setTouch(true); if(boss){S.time=.78;dialogue('リナ',['……空が、明るくなってる。','見て。私たち、本当に夜を越えたんだ。'],()=>toast('VERTICAL SLICE CLEAR — 世界はこの先へ続く',5000))}},650);
}
...
function updateUI(){ $('#hpText').textContent=`${S.hp} / ${S.maxHp}`;$('#mpText').textContent=`${S.mp} / ${S.maxMp}`;$('#hpBar').style.width=`${S.hp/S.maxHp*100}%`;$('#mpBar').style.width=`${S.mp/S.maxMp*100}%`;$('#battleStats').textContent=`HP ${S.hp}　MP ${S.mp}${S.joined?'　✦ リナ AUTO':''}`;$('#itemCount').textContent=`${S.items} 個`; }
...
function field(dt){
  ...
  if(companion){const f=new THREE.Vector3(Math.sin(player.rotation.y),0,Math.cos(player.rotation.y));const side=new THREE.Vector3(f.z,0,-f.x);const target=player.position.clone().addScaledVector(f,-1.7).addScaledVector(side,-.75);const k=1-Math.exp(-dt*5.5);companion.position.x=THREE.MathUtils.lerp(companion.position.x,target.x,k);companion.position.z=THREE.MathUtils.lerp(companion.position.z,target.z,k);companion.position.y=groundY(companion.position.x,companion.position.z)+.22+Math.sin(S.moveT+.8)*.025;const v=player.position.clone().sub(companion.position);companion.rotation.y=THREE.MathUtils.damp(companion.rotation.y,Math.atan2(v.x,v.z),8,dt);companion.userData.gem.rotation.y+=dt*2}
...
function cameraUpdate(dt){
  ...
  if(S.mode==='battle'&&S.enemyObj){const e=S.enemyObj,mid=player.position.clone().lerp(e.position,.52),d=e.position.clone().sub(player.position).normalize(),side=new THREE.Vector3(-d.z,0,d.x),p=mid.clone().addScaledVector(side,8.5).add(new THREE.Vector3(0,5.1,0)).addScaledVector(d,-1.4);camera.position.lerp(p,1-Math.exp(-dt*4));camTarget.lerp(mid.clone().add(new THREE.Vector3(0,1.1,0)),1-Math.exp(-dt*6));camera.lookAt(camTarget)}
