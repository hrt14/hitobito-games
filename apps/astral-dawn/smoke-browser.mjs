...
const waitCombo=()=>waitNoArg(()=>{const b=document.querySelector('.combo-command');return !!b&&!b.disabled&&b.classList.contains('ready');},10000);

async function enterNextNormalBattle(){
  await page.evaluate(()=>{const c=window.__ASTRAL_CORE,p=c.player,e=c.enemies.find(x=>x.visible&&!x.userData.boss&&!x.userData.defeated);if(!e)throw new Error('no normal enemy available');p.position.set(e.position.x,e.position.y,e.position.z);});
  await waitNoArg(()=>window.__ASTRAL_CORE.S.mode==='battle'&&!window.__ASTRAL_CORE.S.enemyObj?.userData.boss,12000);
...
async function useComboAndWaitWins(targetWins){
  ...
  await page.waitForFunction(w=>window.__ASTRAL_CORE.S.wins>=w,targetWins,{timeout:12000});
  await waitNoArg(()=>window.__ASTRAL_CORE.S.mode==='field'||window.__ASTRAL_CORE.S.mode==='dialogue',10000);
...
try{
  ...
  mark('wait-core');
  await waitNoArg(()=>!!window.__ASTRAL_CORE?.renderer,45000);
  await waitNoArg(()=>!document.querySelector('#loading'),15000);
  ...
  await waitNoArg(()=>document.querySelector('#dialogue')?.classList.contains('hidden'),5000);
  await waitNoArg(()=>window.__ASTRAL_CORE?.S?.joined===true,5000);
  await waitNoArg(()=>!!window.__ASTRAL_CORE?.companion,5000);
  ...
  try{await page.waitForFunction(start=>{const p=window.__ASTRAL_CORE?.player?.position;return !!p&&Math.hypot(p.x-start.x,p.z-start.z)>.12;},before,{timeout:7000});}
  ...
  await waitNoArg(()=>window.__ASTRAL_CORE.interactables.find(x=>x.type==='chest')?.obj.userData.opened===true,5000);

  mark('first-battle');
  await enterNextNormalBattle();
  await settleFrames(3);await page.screenshot({path:path.join(out,'battle.png')});
  ...
  await settleFrames(1);await page.keyboard.press('KeyE');
  await waitNoArg(()=>window.__ASTRAL_CORE.S.bossSpawned===true,6000);
  await waitNoArg(()=>window.__ASTRAL_CORE.S.mode==='battle'&&window.__ASTRAL_CORE.S.enemyObj?.userData.boss===true,15000);
  await waitCombo();
  ...
  await settleFrames(3);await page.screenshot({path:path.join(out,'boss.png')});

  mark('defeat-boss');
  await page.click('.combo-command');
  await waitNoArg(()=>window.__ASTRAL_CORE.S.boss===true,12000);
  await page.waitForSelector('#dawnTitle.show',{state:'visible',timeout:12000});
  await settleFrames(2);await page.screenshot({path:path.join(out,'dawn.png')});
  ...
  fs.writeFileSync(path.join(out,'diagnostic.json'),JSON.stringify(result,null,2));console.log(JSON.stringify(result,null,2));
}catch(err){
  const snapshot=await page.evaluate(()=>({url:location.href,body:(document.body?.innerText||'').slice(0,4000),fatal:document.querySelector('#fatal')?.className,loading:!!document.querySelector('#loading'),core:!!window.__ASTRAL_CORE,mode:window.__ASTRAL_CORE?.S?.mode||null,joined:window.__ASTRAL_CORE?.S?.joined||false,wins:window.__ASTRAL_CORE?.S?.wins||0,bossSpawned:window.__ASTRAL_CORE?.S?.bossSpawned||false,boss:window.__ASTRAL_CORE?.S?.boss||false,player:window.__ASTRAL_CORE?.player?{x:window.__ASTRAL_CORE.player.position.x,y:window.__ASTRAL_CORE.player.position.y,z:window.__ASTRAL_CORE.player.position.z}:null,scripts:[...document.scripts].map(x=>x.src).filter(Boolean)})).catch(()=>({}));
  const report={ok:false,stage,error:String(err?.stack||err),errors,messages:messages.slice(-180),snapshot};fs.writeFileSync(path.join(out,'diagnostic.json'),JSON.stringify(report,null,2));await page.screenshot({path:path.join(out,'failure.png'),fullPage:true}).catch(()=>{});console.error(JSON.stringify(report,null,2));throw err;
