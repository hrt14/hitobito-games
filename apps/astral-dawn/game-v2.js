...
function lights(){
  ...
  keyLight.shadow.mapSize.set(2048,2048); Object.assign(keyLight.shadow.camera,{left:-45,right:45,top:45,bottom:-45}); scene.add(keyLight);
...
function terrain(){
  ...
  for(let i=0;i<a.count;i++){ const x=a.getX(i),z=a.getZ(i); a.setY(i,(Math.sin(x*.105)+Math.cos(z*.12))*.34+Math.sin((x+z)*.06)*.42+Math.max(0,(Math.abs(x)-52)*.035)); }
...
function path(){
  ...
  for(let i=0;i<pts.length-1;i++){
    ...
    const p=mesh(new THREE.PlaneGeometry(5,len),mat(C.dirt,1)); p.rotation.x=-Math.PI/2; p.rotation.z=-Math.atan2(dz,dx)+Math.PI/2; p.position.set((x+x2)/2,.05,(z+z2)/2); world.add(p);
  }
}
...
function house(x,z,s=1,r=0){
  ...
  g.position.set(x,0,z); g.scale.setScalar(s); g.rotation.y=r; world.add(g); colliders.push({x,z,r:3*s});
}
...
function lantern(x,z){
  ...
  p.position.y=1.3; l.position.y=2.4; g.add(p,l); g.position.set(x,0,z); world.add(g);
}
...
function ruins(){
  ...
  al.position.y=.4; cr.position.y=2; c.add(al,cr); c.userData.crystal=cr; c.position.set(32,0,-39); world.add(c); scene.userData.shrine=c;
  interactables.push({type:'shrine',obj:c,r:5,text:'祠を調べる'});
  const arch=mesh(new THREE.TorusGeometry(5.7,.65,10,26,Math.PI),mat(0x7b8078)); arch.rotation.z=Math.PI; arch.position.set(32,5.4,-44); world.add(arch);
}
...
function landmark(){
  ...
  b.position.y=8.5; r.position.y=15; r.rotation.x=Math.PI/2; o.position.y=18; g.add(b,r,o); g.position.set(56,2,-67); world.add(g);
}
...
function objectiveBeacon(){
  ...
  g.add(ring,beam); g.position.set(32,.3,-39); g.visible=false; g.userData.ring=ring; world.add(g); scene.userData.beacon=g;
}
...
function hero(){
  ...
  player.position.set(0,.25,4); world.add(player);
}

function npc(x,z,name,color){
  const g=new THREE.Group(),b=mesh(new THREE.CapsuleGeometry(.38,1.1,6,9),mat(color)),h=mesh(new THREE.SphereGeometry(.38,14,10),mat(C.skin)); b.position.y=1.45;h.position.y=2.47;g.add(b,h);g.position.set(x,.25,z);g.userData.name=name;world.add(g);npcs.push(g);interactables.push({type:'npc',obj:g,r:2.2,text:'話す',name});
}
...
function enemy(x,z,boss=false){
  ...
  if(boss){ const ring=mesh(new THREE.TorusGeometry(1.7,.07,8,32),new THREE.MeshBasicMaterial({color:0xa55f68,transparent:true,opacity:.7}),false); ring.rotation.x=Math.PI/2; ring.position.y=.35; g.add(ring); g.userData.aura=ring; }
  g.position.set(x,.15,z); g.userData={boss,hp:boss?300:95,maxHp:boss?300:95,name:boss?'星喰らいのオルグ':'灰角のヴァルグ',home:new THREE.Vector3(x,.15,z),phase:Math.random()*10}; world.add(g); enemies.push(g); return g;
}
...
function battle(e){
  ...
  setTimeout(()=>{ S.mode='battle';S.can=true;e.position.set(player.position.x+3.5,.15,player.position.z-4.6);e.rotation.y=Math.PI;$('#enemyName').textContent=S.enemy.name;$('#battleHud').classList.remove('hidden');updateBattle();log(S.enemy.boss?'古代種が星光を喰らっている！':'魔獣がこちらを睨んでいる！');setTimeout(()=>$('#letterbox').classList.remove('active'),350); },550);
}
...
function gameover(){
  ...
  setTimeout(()=>{S.hp=S.maxHp;S.mp=S.maxMp;player.position.set(1,.25,1);updateUI();if(S.enemyObj?.userData.field){S.enemyObj.position.copy(S.enemyObj.userData.field);S.enemyObj.visible=true}endBattle(false);toast('星の加護で村の入口へ戻った',2300)},1750);
}
...
function field(dt){
  ...
  if(moving){const ang=Math.atan2(move.x,-move.y);player.rotation.y=THREE.MathUtils.damp(player.rotation.y,ang,13,dt);tmp.set(move.x,0,-move.y).multiplyScalar((run?8.6:5.2)*dt);const n=player.position.clone().add(tmp);if(canMove(n))player.position.copy(n);S.moveT+=dt*(run?13:9)}else S.moveT+=dt*2;
