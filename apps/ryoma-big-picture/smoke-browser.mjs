...
async function openLens(key) {
  ...
  await button.click();
  if ((await button.getAttribute('aria-pressed')) !== 'true') fail(`Lens ${key} did not open.`);
}
...
async function finishCurrentWithBig() {
  for (const key of ['purpose', 'people', 'options', 'time']) await openLens(key);
  await pickScale('big');
...
try {
  ...
  if (!(await page.locator('#boardShell').getAttribute('class'))?.includes('open-3')) fail('Board did not visibly expand to open-3 state.');
  ...
  const summary = {
    ...
    tested: ['first-time clarity','moves locked until three lenses','visual board expansion','narrow/small move feedback','big move and four-lens path','verified-history vs abstraction separation','five-round completion and weak-lens result','local best/session persistence','reload/revisit','home/back link','mobile overflow and tap targets'],
    observed: { bigMoves: 4, fullViews: 4, weakestLens: 'time', persistedSessions: 1, persistedBest: 4, consoleErrors },
  };
