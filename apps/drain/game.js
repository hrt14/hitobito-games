(() => {
  ...
  const stages = [
    {id:'bath',name:'家の風呂',short:'風呂',sub:'最初の水抜き',kicker:'STAGE 01',intro:'栓を抜くだけ。水の下に何が残る？',type:'bath',drain:5.2,pump:'排水栓',pumpLevel:18,tease:'次は、泥の中で生き物が動く「小さな沼」',palette:{land:'#b8b5aa',mud:'#93836f',water:'#78c8d5',deep:'#297d91',edge:'#ded8ca'},items:[i('coin','10円玉','落とし物',.32,.74,72,0,'common'),i('trash','シャンプーのキャップ','落とし物',.68,.33,48,0,'common'),i('fish','金魚？','なぜここに',.48,.56,27,1,'rare'),i('mystery','排水口の奥の影','???',.54,.78,8,0,'rare')]},
    {id:'swamp',name:'小さな沼',short:'沼',sub:'泥の中が動いている',kicker:'STAGE 02',intro:'浅くなるほど、泥の住人が姿を現す。',type:'swamp',drain:3.1,pump:'家庭用ポンプ',pumpLevel:30,tease:'次は、魚と人工物が一緒に沈む「古い農業ため池」',palette:{land:'#6f7c4e',mud:'#5c5037',water:'#557e64',deep:'#274b42',edge:'#85915f'},items:[i('frog','トノサマガエル','在来生物',.22,.37,78,1,'common'),i('fish','フナ','在来生物',.70,.33,65,3,'common'),i('eel','ドジョウ','在来生物',.64,.61,50,2,'common'),i('crab','アメリカザリガニ','外来生物',.37,.68,39,2,'rare'),i('trash','長靴','落とし物',.48,.81,18,0,'common'),i('mystery','泥の大きな穴','発見地点',.79,.73,5,0,'rare')]},
    {id:'pond',name:'古い農業ため池',short:'池',sub:'魚と人工物が混ざる',kicker:'STAGE 03',intro:'生活の痕跡まで水面の下に沈んでいる。',type:'pond',drain:2.55,pump:'エンジンポンプ',pumpLevel:44,tease:'次は、流れを迂回させて調べる「山あいの川」',palette:{land:'#758d59',mud:'#756447',water:'#4b9389',deep:'#185b5f',edge:'#9aa86e'},items:[i('fish','ギンブナ','在来生物',.30,.44,80,3,'common'),i('fish','ブラックバス','外来生物',.69,.47,61,4,'rare'),i('turtle','ミシシッピアカミミガメ','外来生物',.48,.26,44,1,'rare'),i('crab','スジエビ','在来生物',.25,.72,31,2,'common'),i('bike','沈んだ自転車','人工物',.60,.73,19,0,'rare'),i('pot','古い陶器','古物',.76,.25,9,0,'rare'),i('fish','巨大なコイ','大型個体',.48,.57,12,1,'rare')]},
    {id:'river',name:'山あいの川',short:'川',sub:'流れを迂回させて調査',kicker:'STAGE 04',intro:'淵が浅くなると、岩陰から一斉に動き出す。',type:'river',drain:2.15,pump:'仮設排水設備',pumpLevel:58,tease:'次は、岸線そのものが変わる「巨大湖」',palette:{land:'#87957e',mud:'#756b58',water:'#4c9ca8',deep:'#1c6477',edge:'#a6b09b'},items:[i('fish','アユ','在来生物',.27,.33,83,5,'common'),i('fish','ニジマス','移入種',.56,.44,69,5,'rare'),i('eel','ウナギ','在来生物',.44,.70,40,3,'rare'),i('crab','モクズガニ','在来生物',.72,.65,29,2,'common'),i('trash','古いタイヤ','人工物',.24,.75,18,0,'common'),i('relic','石碑の一部','歴史物',.73,.28,8,0,'rare'),i('fish','巨大ナマズ','大型個体',.50,.55,11,1,'rare')]},
    {id:'lake',name:'巨大湖',short:'湖',sub:'岸線そのものが変わる',kicker:'STAGE 05',intro:'島が陸続きになる。その先に何がある？',type:'lake',drain:1.8,pump:'大型排水ポンプ群',pumpLevel:72,tease:'次は、干潟の向こうに港が現れる「大きな湾」',palette:{land:'#788d72',mud:'#726753',water:'#357d91',deep:'#123f59',edge:'#9eb09a'},items:[i('fish','コイの群れ','魚類',.29,.44,80,6,'common'),i('fish','ビワコオオナマズ級の大魚','大型個体',.51,.52,56,1,'rare'),i('turtle','大型のカメ','爬虫類',.70,.38,45,1,'common'),i('boat','沈んだ小舟','人工物',.26,.73,27,0,'rare'),i('relic','水没した石段','歴史物',.66,.70,16,0,'rare'),i('pot','古い壺','古物',.79,.57,9,0,'rare'),i('mystery','湖底の直線構造','???',.46,.77,3,0,'rare')]},
    {id:'bay',name:'大きな湾',short:'湾',sub:'干潟の向こうに港が現れる',kicker:'STAGE 06',intro:'海の入口。水が引くと街の裏側が見える。',type:'bay',drain:1.5,pump:'湾岸排水システム',pumpLevel:84,tease:'最後は、水深3,800mまで露出する「北大西洋」',palette:{land:'#8d8878',mud:'#716657',water:'#267b92',deep:'#0d445c',edge:'#aba595'},items:[i('fish','クロダイ','海水魚',.31,.39,85,4,'common'),i('crab','ワタリガニ','甲殻類',.69,.60,58,3,'common'),i('fish','エイ','大型魚',.52,.54,49,2,'rare'),i('boat','沈没した漁船','沈没船',.30,.72,25,0,'rare'),i('trash','海底ケーブル','人工物',.71,.73,14,0,'rare'),i('relic','古い錨','歴史物',.78,.31,8,0,'rare'),i('mystery','巨大な船影','???',.46,.79,2,0,'rare')]},
    {id:'sea',name:'北大西洋',short:'海',sub:'水深3,800mの世界へ',kicker:'FINAL AREA',intro:'大陸棚の先から、海底そのものが現れる。',type:'sea',drain:1.18,pump:'海洋排水システム',pumpLevel:96,tease:'海底のさらに下には、まだ「???」が残っている。',palette:{land:'#555f62',mud:'#39474c',water:'#14566e',deep:'#071f32',edge:'#6d787b'},items:[i('fish','深海魚','深海生物',.24,.42,70,4,'common'),i('fish','巨大なサメ','大型生物',.67,.33,58,3,'rare'),i('mystery','熱水噴出孔','海底地形',.72,.68,36,0,'rare'),i('boat','大型沈没船','沈没船',.32,.70,25,0,'rare'),i('relic','海底ケーブル網','人工物',.62,.78,15,0,'rare'),i('boat','タイタニック号','歴史的沈没船',.48,.56,5,0,'rare'),i('mystery','海溝のさらに下','???',.78,.47,1,0,'rare')]}
  ];
  ...
  const saveKey='hitobito-drain-v3';
  let saved=loadSave();
  ...
  let catchStreak=0,lastCatchAt=0,screenFlash=0;
  ...
  function prepareStage(index){
    stageIndex=index;const s=stages[stageIndex];water=100;draining=false;completed=false;boost=0;found=new Set();firstPromptDone=false;promptItemId=null;lastWaterBucket=100;catchStreak=0;lastCatchAt=0;particles.length=0;ripples.length=0;
  ...
  function updateGoalFromWorld(){
    if(completed)return;
    const s=stages[stageIndex],visible=runtimeItems.filter(x=>!x.caught&&water<=x.reveal);
    if(promptItemId){const p=runtimeItems.find(x=>x.id===promptItemId);setGoal('！',`${p?.name||'光っているもの'}をタップ`,true);return;}
    if(water<=0){const need=requiredFor(s);if(found.size<need)setGoal('＋',`あと${need-found.size}個 タップして調査`,true);else setGoal('✓','水底を調査できた');return;}
    if(visible.length){setGoal('＋',visible.length>1?`${visible.length}個 見えている`:'見えているものをタップ',true);return;}
    const next=runtimeItems.filter(x=>!x.caught&&water>x.reveal).sort((a,b)=>b.reveal-a.reveal)[0];
    const gap=next?water-next.reveal:99;setGoal(gap<12?'…':'↓',gap<12?'水面の下に影…':'さらに水を抜く',gap<12);
  }

  function startDrain(){draining=true;ui.intro.classList.add('hidden');ui.pumpMeter.classList.add('active');setGoal('↓','水位が下がっている');showToast('排水開始');buzz(12);}
  ...
  ui.boostBtn.addEventListener('click',()=>{
    ...
    const prev=water;boost=Math.min(boost+12,26);water=Math.max(0,water-2.4);checkReveal(prev,water);checkWaterMoment();if(water<=0)finishStage();else updateGoalFromWorld();updateHud();
    showToast('水位 -2%');buzz(8);ui.boostBtn.classList.add('flash');ui.waterCard.classList.add('pulse');screenFlash=Math.max(screenFlash,.08);
  ...
  function finishStage(){
    ...
    if(found.size<need){draining=false;ui.pumpMeter.classList.remove('active');updateGoalFromWorld();showToast('水底を調査しよう');buzz([12,35,12]);return;}
    completed=true;draining=false;ui.pumpMeter.classList.remove('active');markRecord();screenFlash=.4;buzz([16,40,24]);spawnBurst(width*.5,height*.5,true,28);
  ...
  canvas.addEventListener('pointerdown',e=>{
    ...
    for(const item of runtimeItems){if(item.caught||water>item.reveal)continue;const q=itemScreen(item),dist=Math.hypot(p.x-q.x,p.y-q.y);const radius=item.kind==='fish'?44:40;if(dist<radius&&dist<best){target=item;best=dist;}}
    if(target)catchItem(target);else{spawnTapRipple(p.x,p.y);buzz(4);if(promptItemId)showToast('光っているものをタップ');}
  });

  function catchItem(target){
    const now=performance.now(),p=itemScreen(target);catchStreak=now-lastCatchAt<2200?catchStreak+1:1;lastCatchAt=now;
    target.caught=true;found.add(target.id);markRecord();spawnBurst(p.x,p.y,target.rarity==='rare',target.rarity==='rare'?22:12);screenFlash=Math.max(screenFlash,target.rarity==='rare'?.24:.09);buzz(target.rarity==='rare'?[12,30,24]:10);
    const streak=catchStreak>=2?` · ${catchStreak}連続`:'';showToast(`${target.rarity==='rare'?'★ 大発見：':'＋ '}${target.name}${streak}`);updateHud();updateDiscoveries();
    if(target.id===promptItemId){promptItemId=null;draining=true;ui.pumpMeter.classList.add('active');showToast('調査成功 → 排水再開');}
    if(water<=0)finishStage();else updateGoalFromWorld();
  }

  function spawnTapRipple(x,y){ripples.push({x,y,t:0});}
  function spawnBurst(x,y,rare,count){for(let n=0;n<count;n++){const a=Math.random()*Math.PI*2,s=(rare?32:22)*(0.5+Math.random());particles.push({x,y,vx:Math.cos(a)*s,vy:Math.sin(a)*s-8,t:0,life:.45+Math.random()*.35,size:rare?2.5:1.8,rare});}}
  function itemScreen(item){return{x:(item.x+item.ox)*width,y:(item.y+item.oy)*height};}

  function loop(now){
    const dt=Math.min((now-last)/1000,.05);last=now;
    if(draining&&water>0){const s=stages[stageIndex],prev=water;water-=dt*(s.drain+boost*.13);boost=Math.max(0,boost-dt*5.1);water=Math.max(0,water);checkReveal(prev,water);checkWaterMoment();if(water<=0)finishStage();else if(!promptItemId)updateGoalFromWorld();updateHud();}
    animateItems(dt,now);updateEffects(dt);draw(now);requestAnimationFrame(loop);
  }

  function checkReveal(prev,current){
    const newly=runtimeItems.filter(x=>!x.caught&&!x.noticed&&prev>x.reveal&&current<=x.reveal);newly.forEach(x=>x.noticed=true);
    if(!newly.length)return;
    const rare=newly.find(x=>x.rarity==='rare');screenFlash=Math.max(screenFlash,rare?.18:.06);buzz(rare?[8,30,12]:7);
    if(!firstPromptDone){firstPromptDone=true;promptItemId=newly[0].id;draining=false;ui.pumpMeter.classList.remove('active');updateGoalFromWorld();showToast('何か出た！');}
    else{showToast(rare?'大きな影が出た…':'新しいものが出てきた');updateGoalFromWorld();}
  }

  function checkWaterMoment(){
    const bucket=water>75?100:water>50?75:water>25?50:water>8?25:0;if(bucket===lastWaterBucket)return;lastWaterBucket=bucket;
    ui.waterCard.classList.remove('pulse');void ui.waterCard.offsetWidth;ui.waterCard.classList.add('pulse');screenFlash=Math.max(screenFlash,.12);buzz(8);
  ...
  function updateEffects(dt){
    for(const r of ripples)r.t+=dt;while(ripples.length&&ripples[0].t>.55)ripples.shift();
    for(let n=particles.length-1;n>=0;n--){const p=particles[n];p.t+=dt;p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=24*dt;if(p.t>p.life)particles.splice(n,1);}
    screenFlash=Math.max(0,screenFlash-dt*1.8);
  }

  function draw(now){
    const s=stages[stageIndex];ctx.clearRect(0,0,width,height);drawGround(s);drawStageIdentity(s,now);drawExposedWorld(s,now);drawWater(s,now);drawScenery(s);drawPumpRig(s,now);drawItems(s,now);drawRipples();drawParticles();drawAtmosphere(s,now);if(screenFlash>0){ctx.save();ctx.globalAlpha=screenFlash*.22;ctx.fillStyle='#eaffb0';ctx.fillRect(0,0,width,height);ctx.restore();}
  }
  ...
  function drawStageIdentity(s,now){
    ctx.save();
    if(s.type==='bath'){
      ctx.strokeStyle='rgba(255,255,255,.12)';ctx.lineWidth=1;for(let x=0;x<width;x+=28){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,height);ctx.stroke();}for(let y=0;y<height;y+=28){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(width,y);ctx.stroke();}
      ctx.fillStyle='#c7c5bc';ctx.fillRect(width*.70,height*.08,width*.16,10);ctx.fillRect(width*.79,height*.08,10,height*.10);ctx.fillStyle='#8b8c87';ctx.beginPath();ctx.arc(width*.79,height*.19,7,0,Math.PI*2);ctx.fill();
    }else if(s.type==='swamp'){
      ctx.strokeStyle='rgba(37,54,30,.8)';ctx.lineWidth=3;for(let n=0;n<16;n++){const x=width*(.05+(n%8)*.12),y=height*(n<8?.18:.84);ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x+Math.sin(n)*5,y-(n<8?1:-1)*(18+n%4*5));ctx.stroke();}
      ctx.fillStyle='rgba(67,52,33,.72)';ctx.save();ctx.translate(width*.72,height*.14);ctx.rotate(-.28);ctx.fillRect(-38,-5,76,10);ctx.restore();
    }else if(s.type==='pond'){
      ctx.fillStyle='rgba(95,75,52,.82)';ctx.fillRect(width*.07,height*.14,width*.25,8);for(let n=0;n<4;n++)ctx.fillRect(width*(.09+n*.065),height*.14,5,35);
      ctx.strokeStyle='rgba(99,91,69,.55)';ctx.lineWidth=2;for(let n=0;n<5;n++){ctx.beginPath();ctx.moveTo(width*(.60+n*.07),height*.08);ctx.lineTo(width*(.60+n*.07),height*.18);ctx.stroke();}
    }else if(s.type==='river'){
      ctx.fillStyle='rgba(76,75,66,.8)';for(let n=0;n<12;n++){ctx.beginPath();ctx.ellipse(width*(.06+(n%6)*.18),height*(n<6?.14:.86),12+n%3*4,7+n%2*3,n,0,Math.PI*2);ctx.fill();}
    }else if(s.type==='lake'){
      ctx.fillStyle='rgba(57,76,50,.58)';ctx.beginPath();ctx.ellipse(width*.50,height*.18,width*.14,height*.07,-.2,0,Math.PI*2);ctx.fill();ctx.fillStyle='rgba(43,58,40,.65)';for(let n=0;n<8;n++){ctx.beginPath();ctx.arc(width*(.42+n*.025),height*(.17+Math.sin(n)*.018),6+n%3,0,Math.PI*2);ctx.fill();}
    }else if(s.type==='bay'){
      ctx.strokeStyle='rgba(63,67,67,.85)';ctx.lineWidth=4;for(let n=0;n<3;n++){const x=width*(.10+n*.13);ctx.beginPath();ctx.moveTo(x,height*.08);ctx.lineTo(x,height*.20);ctx.lineTo(x+24,height*.12);ctx.stroke();}
      ctx.fillStyle='rgba(70,75,76,.72)';for(let n=0;n<5;n++)ctx.fillRect(width*(.62+n*.065),height*(.06+(n%2)*.035),18,height*(.10+(n%3)*.025));
    }else if(s.type==='sea'){
      ctx.strokeStyle='rgba(190,220,225,.11)';ctx.lineWidth=1;for(let n=0;n<6;n++){ctx.beginPath();ctx.ellipse(width*.5,height*.55,width*(.18+n*.055),height*(.11+n*.045),-.18,0,Math.PI*2);ctx.stroke();}
      ctx.fillStyle='rgba(180,196,197,.72)';ctx.save();ctx.translate(width*.13,height*.12);ctx.rotate(.16);ctx.fillRect(-22,-5,44,10);ctx.fillStyle='rgba(35,45,47,.8)';ctx.fillRect(-8,-12,14,7);ctx.restore();
    }
    ctx.restore();
  }
  ...
  function drawLowWaterLandmark(s,now){
    ctx.globalAlpha=Math.min(.82,(27-water)/18);
  ...
  function drawScenery(s){
    ctx.save();
    if(s.type==='bath'){ctx.strokeStyle='rgba(255,255,255,.38)';ctx.lineWidth=7;roundRectStroke(width*.15,height*.18,width*.70,height*.62,34);ctx.fillStyle='#59605e';ctx.beginPath();ctx.arc(width*.78,height*.73,10,0,Math.PI*2);ctx.fill();ctx.fillStyle='#222928';ctx.beginPath();ctx.arc(width*.78,height*.73,5,0,Math.PI*2);ctx.fill();}
    else{const trees=s.type==='sea'?0:s.type==='bay'?4:9;for(let n=0;n<trees;n++){const side=n%2?1:-1,x=width*(.08+(n%3)*.07),px=side===1?width-x:x,py=height*(.12+(n%5)*.18);drawTree(px,py,9+(n%3)*3);}if(s.type==='river'){ctx.fillStyle='rgba(90,85,75,.72)';ctx.fillRect(width*.08,height*.48,width*.84,9);for(let n=0;n<7;n++)ctx.fillRect(width*(.12+n*.12),height*.45,5,24);}if(s.type==='bay'){ctx.fillStyle='#474d4d';ctx.fillRect(width*.04,height*.08,width*.27,height*.14);ctx.fillStyle='#616969';for(let n=0;n<4;n++)ctx.fillRect(width*(.06+n*.06),height*.04,15,height*.09);}if(s.type==='sea'){ctx.fillStyle='rgba(120,145,150,.38)';ctx.beginPath();ctx.moveTo(0,height*.22);ctx.lineTo(width*.20,height*.13);ctx.lineTo(width*.30,height*.25);ctx.lineTo(width*.42,height*.18);ctx.lineTo(width*.54,height*.30);ctx.lineTo(0,height*.34);ctx.closePath();ctx.fill();}}
    ctx.restore();
  }

  function drawPumpRig(s,now){
    if(stageIndex===0)return;
    const units=Math.min(4,1+Math.floor(stageIndex/2)),baseX=width*.07,baseY=height*.80,pulse=draining?(Math.sin(now*.012)*.5+.5):0;
    ctx.save();ctx.lineCap='round';ctx.strokeStyle=`rgba(38,55,54,${.75+pulse*.18})`;ctx.lineWidth=6+stageIndex*.7;ctx.beginPath();ctx.moveTo(baseX+22,baseY);ctx.bezierCurveTo(width*.16,height*.70,width*.22,height*.68,width*.29,height*.66);ctx.stroke();
    for(let n=0;n<units;n++){const x=baseX+n*24,y=baseY+(n%2)*7,scale=1+stageIndex*.07;ctx.save();ctx.translate(x,y);ctx.scale(scale,scale);ctx.fillStyle='#273435';ctx.fillRect(-8,-8,18,16);ctx.fillStyle=draining?'#9bd9cd':'#657a77';ctx.fillRect(-5,-5,8,4);ctx.strokeStyle='#9aa6a1';ctx.lineWidth=2;ctx.beginPath();ctx.arc(7,0,5,0,Math.PI*2);ctx.stroke();ctx.restore();}
    if(stageIndex>=5){ctx.fillStyle='rgba(57,67,68,.85)';for(let n=0;n<2;n++){const x=width*(.12+n*.08);ctx.fillRect(x,height*.69,14,height*.13);ctx.fillStyle=draining?'rgba(216,239,119,.55)':'rgba(150,165,160,.35)';ctx.fillRect(x+4,height*.71,6,8);ctx.fillStyle='rgba(57,67,68,.85)';}}
    ctx.restore();
  }
  ...
  function drawItems(s,now){
    const need=requiredFor(s),endFocus=water<=0&&found.size<need;
    runtimeItems.forEach((item,n)=>{
      if(item.caught)return;const p=itemScreen(item);
      if(water>item.reveal){const delta=water-item.reveal;if(delta<22){ctx.save();ctx.globalAlpha=(22-delta)/22*.34;ctx.fillStyle='#071819';ctx.beginPath();ctx.ellipse(p.x,p.y,18+(item.rarity==='rare'?14:2),7,Math.sin(item.phase),0,Math.PI*2);ctx.fill();ctx.restore();}return;}
      const emerge=Math.min(1,(item.reveal-water+4)/10),prompted=item.id===promptItemId,focused=prompted||endFocus;ctx.save();ctx.translate(p.x,p.y);ctx.globalAlpha=.42+.58*emerge;
      if(item.rarity==='rare'||focused){ctx.strokeStyle=focused?'rgba(255,255,255,.92)':'rgba(216,239,119,.62)';ctx.lineWidth=focused?3:1.5;ctx.beginPath();ctx.arc(0,0,(focused?28:21)+Math.sin(now*.006+n)*3,0,Math.PI*2);ctx.stroke();}
      if(focused){ctx.globalAlpha=.14;ctx.fillStyle='#d8ef77';ctx.beginPath();ctx.arc(0,0,36+Math.sin(now*.006)*4,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;ctx.fillStyle='rgba(255,255,255,.9)';ctx.font='900 9px system-ui';ctx.textAlign='center';ctx.fillText('TAP',0,-34-Math.sin(now*.006)*3);}
  ...
  function drawParticles(){for(const p of particles){const q=p.t/p.life;ctx.save();ctx.globalAlpha=(1-q)*.9;ctx.fillStyle=p.rare?'#e8ff91':'#d7f6ef';ctx.beginPath();ctx.arc(p.x,p.y,p.size*(1-q*.3),0,Math.PI*2);ctx.fill();ctx.restore();}}
