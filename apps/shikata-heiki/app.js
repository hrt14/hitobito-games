...
let deck=[],index=0,earned=0,currentLevel=0,startX=0,deltaX=0,dragging=false;
const saved=Number(localStorage.getItem('unplannedToleranceXp')||0);$('#xp').textContent=saved;
...
function begin(){deck=shuffle();index=0;earned=0;currentLevel=levelFromXp(saved);show('training');render()}
function render(){const s=stages[currentLevel],item=deck[index];$('#levelLabel').textContent=s.label;$('#cue').textContent=s.cue;$('#echo').textContent=s.echo;$('#count').textContent=index+1;$('#progress').style.width=((index+1)/deck.length*100)+'%';$('#tag').textContent=item.tag;$('#scenario').textContent=item.text;$('#thought').textContent=item.thought;resetCard()}
...
function fly(dir=1){const c=$('#card');const item=deck[index];earned+=item.xp;c.style.transition='transform .28s cubic-bezier(.2,.8,.2,1),opacity .25s';c.style.transform=`translate3d(${dir*125}vw,-6vh,0) rotate(${dir*18}deg)`;c.style.opacity='0';if(navigator.vibrate)navigator.vibrate(12);setTimeout(next,220)}
function next(){index++;if(index>=deck.length)return finish();const projected=saved+earned;currentLevel=levelFromXp(projected);render()}
function finish(){const total=saved+earned;localStorage.setItem('unplannedToleranceXp',String(total));$('#xp').textContent=total;const lv=levelFromXp(total);$('#earned').textContent=earned;$('#resultTitle').textContent=stages[lv].label.replace(/^LEVEL \d · /,'')+'へ。';$('#resultCopy').textContent=lv===3?'目標は「気にしないようにする」ことではなく、そもそも反応しなくなること。':'思い通りにならなかった回数を、耐性の経験値に変えていく。';$('#path').innerHTML=stages.map((s,i)=>`<div class="${i<=lv?'done':''}">${i+1}<br>${s.label.split('· ')[1]}</div>`).join('');show('result')}
...
$('#realStart').onclick=()=>{const text=$('#realText').value.trim();if(!text){$('#realText').focus();return}deck=[{tag:'いま',text,thought:'「こうなってほしかった」',xp:12}];index=0;earned=0;currentLevel=levelFromXp(saved);show('training');render()};
const card=$('#card');card.addEventListener('pointerdown',e=>{dragging=true;startX=e.clientX;deltaX=0;card.setPointerCapture?.(e.pointerId);card.classList.add('dragging')});card.addEventListener('pointermove',e=>{if(!dragging)return;deltaX=e.clientX-startX;card.style.transform=`translate3d(${deltaX}px,0,0) rotate(${deltaX/18}deg)`;card.style.opacity=String(Math.max(.35,1-Math.abs(deltaX)/320))});function release(){if(!dragging)return;dragging=false;card.classList.remove('dragging');Math.abs(deltaX)>72?fly(deltaX>0?1:-1):resetCard()}card.addEventListener('pointerup',release);card.addEventListener('pointercancel',release);card.addEventListener('keydown',e=>{if(e.key==='ArrowLeft')fly(-1);if(e.key==='ArrowRight'||e.key==='Enter')fly(1)});
