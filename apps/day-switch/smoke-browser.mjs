...
const fontSizes=await page.locator('#app button,#app input,#app .stage-copy,#app .lead,#app .hint').evaluateAll(nodes=>nodes.filter(n=>{const r=n.getBoundingClientRect();return r.width>0&&r.height>0}).map(n=>parseFloat(getComputedStyle(n).fontSize)));
if(fontSizes.some(v=>v<13))throw new Error(`mobile app text below 13px: ${Math.min(...fontSizes)}`);
