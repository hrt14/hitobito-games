(() => {
  ...
  polishStyle.textContent = '@media(max-height:760px) and (max-width:520px){#gameScreen.answered .scenario p,#gameScreen.answered .metric-row{display:none}#gameScreen.answered .scenario{padding-bottom:11px}#gameScreen.answered .scenario h2{margin-bottom:0}#gameScreen.answered .choices{gap:5px}#gameScreen.answered .choice{min-height:44px;padding-top:8px;padding-bottom:8px}#gameScreen.answered .feedback{margin-top:7px;padding:11px 12px}#gameScreen.answered .feedback p{margin:5px 0}#gameScreen.answered .next{min-height:44px;margin-top:7px}}';
  ...
  const scenarios = [
    ...
    {tag:'広告',q:'コマースアドマネージャーのクリックは増えた。でも利益が悪化。',ctx:'クリック数は好調。売上も少し増えたが、広告費の伸びの方が大きい。',m:['クリック +55%','売上 +18%','粗利率 低下'],choices:[
      ['入札・予算を一律でさらに上げる',0],['商品別に売上・粗利・広告効率を見て配分を変える',2],['広告を全部停止して自然流入だけにする',1]],best:1,
      fb:'検索結果やカテゴリリストに出るクリック課金型広告でも、売上だけでなく商品別の利益まで見て配分する。効率の良い商品へ寄せ、悪い商品はページ・条件改善とセットで見直す。',lesson:'型：売上↑でも広告費↑↑なら勝ちとは限らない。売上 → 粗利 → 広告費の順に残るお金を見る。',d:[[8,0,-9,0],[5,2,10,0],[-8,0,5,0]]},
    ...
    {tag:'ポイント',q:'ポイント施策を強くしたい。どう設計する？',ctx:'利益率の高い商品と低い商品が混在。全商品一律の還元にすると採算が不安。',m:['商品A 粗利高','商品B 粗利低','予算 限定'],choices:[
      ...
      fb:'Yahoo!ショッピングでは商品ごとにポイント倍率を設定できる。だからこそ、利益率や役割に合わせて使い分けるのが実務的。',lesson:'型：一律施策より商品別採算。ポイントは「無料の売上」ではなく原資を伴う販促。',d:[[5,3,10,1],[3,1,-4,0],[7,1,-10,0]]},
  ...
  const state = {round:0,score:0,streak:0,kpis:{traffic:50,cvr:50,profit:50,repeat:50},deck:[],wrong:[],timer:null,sound:true,answered:false};
  ...
  function showScreen(name){[els.start,els.game,els.result].forEach(x=>x.classList.remove('active'));els.game.classList.remove('answered');els[name].classList.add('active');window.scrollTo(0,0);}
