(() => {
  ...
  function showChain() {
    ...
    $('chainTimeline').innerHTML = [
      stepHtml(1, product ? '返品の一言と町工場がつながる。' : '需要か製造のどちらかが消えていた。', product ? '「毎月届くなら欲しい」という声を、小ロット試作で確かめられた。' : '商品化の最初の橋が架からず、アイデアはメモのまま終わった。', product),
      stepHtml(2, brand ? 'デザイナーが、商品を「選ばれる理由」に変える。' : '商品はできても、伝わり方が弱い。', brand ? '以前の細い接点から相談でき、見た目ではなく体験全体を組み直した。' : '売れる理由をつくる接点がなく、価格競争から抜けにくい。', brand),
      stepHtml(3, retail ? '飛行機で会った人から、200店舗のテストへ。' : '小売の扉は開かなかった。', retail ? '雑談の相手が後に商品責任者だと判明。小さなテスト販売が全国へ広がる。' : '肩書きが見えない時点で切れた接点は、後から探しても戻らない。', retail),
      stepHtml(4, recurring ? 'エンジニアが、単発売りを仕組みに変える。' : '伸びるほど、手作業が詰まり始める。', recurring ? '在庫・受注・継続課金がつながり、売上増が作業量増に直結しなくなる。' : '事業が伸びるほど運用負荷が増え、成長の速度が落ちる。', recurring),
      stepHtml(5, global ? '通訳だった人の接点から、海外テストへ。' : '国内で強くても、次の市場へ出る橋がない。', global ? '最初は商談相手ですらなかった人が、東南アジアの流通につないだ。' : '海外の具体的な最初の一歩がなく、国内だけで成長が頭打ちになる。', global)
    ].join('');
