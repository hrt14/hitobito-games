...
function scriptFor(slug) {
  ...
  return [
    ...
    'const host=document.createElement("div"); host.id="levelup-feedback-root"; host.innerHTML="<button id=\\"lu-fb-fab\\" type=\\"button\\">改善要望</button><div id=\\"lu-fb-overlay\\" aria-hidden=\\"true\\"><section id=\\"lu-fb-panel\\" role=\\"dialog\\" aria-modal=\\"true\\"><button id=\\"lu-fb-close\\" type=\\"button\\" aria-label=\\"閉じる\\">×</button><h2>改善要望を送る</h2><p id=\\"lu-fb-meta\\"></p><div id=\\"lu-fb-types\\"><button data-type=\\"improvement\\" class=\\"on\\">改善</button><button data-type=\\"confusing\\">わかりにくい</button><button data-type=\\"bug\\">バグ</button><button data-type=\\"idea\\">アイデア</button></div><textarea id=\\"lu-fb-message\\" maxlength=\\"800\\" placeholder=\\"どうしたらもっと良くなる？\\"></textarea><p id=\\"lu-fb-status\\"></p><button id=\\"lu-fb-send\\" type=\\"button\\">送信する</button></section></div>";',
    'const css=document.createElement("style"); css.textContent="#levelup-feedback-root{font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif}#lu-fb-fab{position:fixed;right:max(12px,env(safe-area-inset-right));bottom:max(12px,env(safe-area-inset-bottom));z-index:2147483646;min-height:44px;padding:0 14px;border-radius:999px;border:1px solid rgba(216,255,91,.42);background:rgba(12,15,10,.94);color:#d8ff5b;font-weight:900}#lu-fb-overlay{position:fixed;inset:0;z-index:2147483647;background:rgba(4,5,4,.78);display:none;align-items:flex-end;justify-content:center;padding:14px;backdrop-filter:blur(8px)}#lu-fb-overlay.open{display:flex}#lu-fb-panel{position:relative;width:min(560px,100%);border:1px solid rgba(216,255,91,.28);border-radius:24px;background:#0f130d;color:#f6f8f1;padding:18px}#lu-fb-close{position:absolute;right:12px;top:12px;width:36px;height:36px;border-radius:50%;border:1px solid rgba(255,255,255,.13);background:#171b15;color:#fff;font-size:20px}#lu-fb-panel h2{margin:0 44px 6px 0;font-size:20px}#lu-fb-meta{font-size:11px;color:#aab2a5;line-height:1.5}#lu-fb-types{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin:12px 0}#lu-fb-types button{min-height:40px;border-radius:12px;border:1px solid rgba(255,255,255,.12);background:#151914;color:#dfe4da;font-weight:800}#lu-fb-types button.on{border-color:#d8ff5b;color:#e9ff9c}#lu-fb-message{width:100%;min-height:130px;border-radius:15px;border:1px solid rgba(255,255,255,.14);background:#0a0d09;color:#fff;padding:12px;font:inherit}#lu-fb-status{min-height:18px;font-size:11px;color:#d8ff5b}#lu-fb-send{width:100%;min-height:48px;border:0;border-radius:14px;background:#d8ff5b;color:#11150d;font-weight:950}@media(max-width:520px){#lu-fb-types{grid-template-columns:1fr 1fr}}"; document.head.appendChild(css); document.body.appendChild(host);',
    ...
    'send.addEventListener("click",async()=>{if(sending)return;const message=textarea.value.trim();if(message.length<2){status.textContent="もう少しだけ具体的に書いてください。";return}sending=true;send.disabled=true;send.textContent="送信中…";try{const payload={type:selectedType,message,appSlug,appTitle:document.querySelector("h1")?.textContent?.trim()||document.title,pageTitle:document.title,pagePath:location.pathname+(location.hash||""),screenLabel:document.querySelector(".screen.active h1,.screen.active h2,.view.active h1,.view.active h2,h2,h1")?.textContent?.trim()||document.title,buildSha,viewport:Math.round(innerWidth)+"x"+Math.round(innerHeight)};const r=await fetch(endpoint,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});const result=await r.json().catch(()=>({}));if(!r.ok||!result.ok)throw new Error(result.error||"SEND_FAILED");textarea.value="";status.textContent="送信しました。";send.textContent="送信しました ✓";setTimeout(()=>{hide();send.textContent="送信する"},800)}catch(e){console.warn("[LEVEL UP feedback] submit failed",e);status.textContent=e?.message==="RATE_LIMIT"?"少し時間を空けてください。":"送信できませんでした。";send.textContent="もう一度送る"}finally{sending=false;send.disabled=false}});',
    '})();',
    '</script>'
  ].join('\n');
}
