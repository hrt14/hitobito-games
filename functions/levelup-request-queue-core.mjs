...
export function validLevelupCreationRequest(raw) {
...
`- 似た既存アプリがあっても制作前の重複チェックでは止めない。依頼ごとに最適な体験を作り、必要なら制作後に品質・利用実績で整理する。\n` +
...
function queueApi(repo, token) {
  if (!repo.includes('/')) throw new Error('REQUEST_QUEUE_REPOSITORY is required');
  if (!token) throw new Error('PRIVATE_REQUEST_TOKEN is required');
  ...
  async function github(path, options = {}) {
    ...
    if (!response.ok) {
      throw new Error(`GitHub ${options.method || 'GET'} ${path} -> ${response.status}`);
    }
  ...
  return { prepare, findExistingIssue, createIssue };
}
...
async function claimRequest(db, userRef, request) {
  const indexRef = db.collection('levelupCreationRequestIndex').doc(request.id);
  const now = Date.now();
  const leaseUntil = now + 2 * 60 * 1000;
  return db.runTransaction(async (tx) => {
    const snap = await tx.get(indexRef);
    const data = snap.exists ? (snap.data() || {}) : {};
    if (data.githubIssueNumber) return { claimed: false, indexRef, data, reason: 'already-queued' };
    const leaseMs = data.leaseExpiresAt?.toMillis?.() || 0;
    if (data.status === 'queuing' && leaseMs > now) return { claimed: false, indexRef, data, reason: 'leased' };
    const patch = {
      userId: userRef.id,
      requestId: request.id,
      status: 'queuing',
      leaseExpiresAt: Timestamp.fromMillis(leaseUntil),
      updatedAt: FieldValue.serverTimestamp(),
    };
    if (!snap.exists) patch.createdAt = FieldValue.serverTimestamp();
    tx.set(indexRef, patch, { merge: true });
    return { claimed: true, indexRef, data: patch, reason: 'claimed' };
  });
}

async function finalizeQueued(db, userRef, request, indexRef, issue) {
  await indexRef.set({
    userId: userRef.id,
    requestId: request.id,
    status: 'building',
  ...
  await updateUserRequest(userRef, request.id, {
    status: 'building',
    githubIssueNumber: issue.number,
...
async function releaseClaim(indexRef) {
  await indexRef.set({
    status: 'pending',
    leaseExpiresAt: FieldValue.delete(),
...
export async function syncLevelupCreationRequests({ db, repo, token, userDocs, maxPerUser = 5 }) {
  const api = queueApi(repo, token);
  await api.prepare();
  let synced = 0;
  ...
  for (const userDoc of userDocs) {
    ...
    for (const request of requests.filter(validLevelupCreationRequest).slice(0, maxPerUser)) {
      const claim = await claimRequest(db, userDoc.ref, request);
      if (!claim.claimed) {
        if (claim.data?.githubIssueNumber) {
          await updateUserRequest(userDoc.ref, request.id, {
            status: claim.data.status || 'building',
            githubIssueNumber: claim.data.githubIssueNumber,
          });
        }
        continue;
      }
      ...
      try {
        let issue = await api.findExistingIssue(request.id);
        if (!issue) issue = await api.createIssue(request);
        await finalizeQueued(db, userDoc.ref, request, claim.indexRef, issue);
        synced += 1;
      } catch (error) {
        await releaseClaim(claim.indexRef).catch(() => {});
        throw error;
      }
    }
  }
...
export async function syncSpecificLevelupCreationRequests({ db, repo, token, userDoc, requestIds }) {
  const wanted = new Set(requestIds || []);
  const data = userDoc.data() || {};
  const raw = data.creationRequests;
  const requests = raw && typeof raw === 'object' && !Array.isArray(raw)
    ? Object.values(raw).filter((request) => wanted.has(request?.id))
    : [];
  if (!requests.some(validLevelupCreationRequest)) return 0;
  const syntheticUserDoc = { id: userDoc.id, ref: userDoc.ref, data: () => ({ creationRequests: Object.fromEntries(requests.map((r) => [r.id, r])) }) };
  return syncLevelupCreationRequests({ db, repo, token, userDocs: [syntheticUserDoc], maxPerUser: requests.length });
}
