...
const emailNotification = { state: 'disabled', reason: 'mypage-only' };
await indexRef.set({
  status,
  resultUrl,
  ...
  emailNotification,
  completionChannel: 'my-quests',
  updatedAt: FieldValue.serverTimestamp(),
  completedAt: FieldValue.serverTimestamp()
}, { merge: true });

console.log(`[OSG] request ${requestId} -> ${status}; completion is available in MY QUESTS; email disabled`);
