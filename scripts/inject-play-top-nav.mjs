...
<style id="hitobito-play-nav-style">
  #hitobito-play-nav{position:fixed;z-index:2147483647;top:max(10px,env(safe-area-inset-top));left:max(8px,env(safe-area-inset-left));font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Hiragino Sans","Yu Gothic UI",sans-serif;color:#fff}
  ...
  @media(max-width:720px){#hitobito-play-nav{top:max(8px,env(safe-area-inset-top));left:max(7px,env(safe-area-inset-left))}#hitobito-play-nav-toggle{width:42px;height:42px}}
...
console.log('Injected Play hamburger navigation into Cloudflare index.');
