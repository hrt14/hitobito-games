import './register-negotiator-sleep-meta.mjs';
import './register-negotiator-move-meta.mjs';
import './register-two-tasks-only-meta.mjs';
import './register-personal-workflow-meta.mjs';
import './register-jinsei-fukusen-meta.mjs';
import './register-life-rpg-status-meta.mjs';
import { GAME_META } from './playtest-catalog.mjs';

GAME_META['income-akinator'] = [
  'levelup',
  '職業名も年収も直接聞かず、「はい / いいえ」だけで働き方の特徴を絞り込み、最後に職業とゲーム内推定年収を当てにいく推理診断。',
];

GAME_META['solo-survival'] = [
  'nature',
  '群れない動物の「隠れる・獲物を独占する・争いを避ける・夜に動く」を、8夜の3択判断で体験する単独行動サバイバル。',
];
