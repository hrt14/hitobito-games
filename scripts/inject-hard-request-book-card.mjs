...
if (!fs.existsSync(homePath) || !fs.existsSync(catalogPath)) {
  throw new Error('LEVEL UP home/catalog not found for hard-request book card.');
}

const slug = 'hard-request';
const title = '相手が嫌がりそうで言えない人の「必要なお願いをする」練習';
const obi = '責めず、逃げず、NOも背負わず。必要なことを丁寧に伝える。';
...
if (!game) throw new Error('hard-request missing from LEVEL UP catalog.');
game.title = title;
...
if (!match) throw new Error('hard-request card missing from LEVEL UP home.');

let body = match[2].replace(/<p class="book-obi">[\s\S]*?<\/p>\s*/g, '');
if (!/<h2\b[^>]*>[\s\S]*?<\/h2>/.test(body)) throw new Error('hard-request card title missing.');
...
if (!finalHtml.includes(`data-game="${slug}"`) || !finalHtml.includes(`<p class="book-obi">${escapeHtml(obi)}</p>`)) {
  throw new Error('hard-request book card injection failed.');
}
console.log('[Firebase] hard-request title + obi copy injected.');
