...
function assertText(value, label, min = 12) {
...
function validateReport(reportPath, expectedSlug = null) {
  ...
  if (!assertText(report.funStatement, `${reportPath}: funStatement`, 20)) ok = false;
  ...
  if (Array.isArray(report.coreLoop)) {
    report.coreLoop.forEach((step, index) => {
      if (!assertText(step, `${reportPath}: coreLoop[${index}]`, 5)) ok = false;
    });
  }
  ...
  if (report.meaningfulDecision) {
    if (!assertText(report.meaningfulDecision.tradeoff, `${reportPath}: meaningfulDecision.tradeoff`, 15)) ok = false;
    if (!assertText(report.meaningfulDecision.consequence, `${reportPath}: meaningfulDecision.consequence`, 15)) ok = false;
  }
  if (!assertText(report.highestMoment, `${reportPath}: highestMoment`, 15)) ok = false;
  if (!assertText(report.oneMoreRunReason, `${reportPath}: oneMoreRunReason`, 15)) ok = false;
  ...
  if (report.playtest) {
    ...
    for (const key of TEST_KEYS) {
      ...
      if (test) {
        ...
        if (!assertText(test.evidence, `${reportPath}: playtest.${key}.evidence`, 20)) ok = false;
      }
    }
  }
  ...
  if (report.scoreEvidence) {
    for (const key of SCORE_KEYS) {
      if (!assertText(report.scoreEvidence[key], `${reportPath}: scoreEvidence.${key}`, 20)) ok = false;
    }
  }

  const verified = Date.parse(report.verifiedAt);
  mark(Number.isFinite(verified), 'verifiedAt must be a valid ISO date/time from the actual playtest.');
...
function isGameplayFile(file) {
  ...
  return GAMEPLAY_EXTENSIONS.has(ext);
}
...
function validateDiff(base, head) {
  ...
  for (const slug of playSlugs) {
    ...
    const reportChanged = changed.includes(reportRelative);
    if (!reportChanged) {
